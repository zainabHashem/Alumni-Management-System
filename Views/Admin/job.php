...
$Events = $EventController->showPost("job");
$totalreact = $EventController->totalReact(0);
$SavedEvents = $EventController->showSavedPosts("job");
if (isset($_POST['createPost'])) {
  if (!empty($_POST['postDescription']) || !empty($_POST['uploadedLink']) || !empty($_POST['uploadedFile']) || !empty($_POST['uploadedImage']) ) {
    ...
    $job = new Job("null", "$description","$image","$link","$file");
    if ($EventController->insertPost($job,$job->getPostType())) {
